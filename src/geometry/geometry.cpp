...
int main() {
	...
	for (int i = 0; i < u; ++i) {
		...
		cin >> x >> y;
		point cur = {x, y};
		int cres = point_in_pgon(cur, pg);
		if (cres == 0) cout << "QAQ\n";
		else if (cres == 1) cout << "10-4\n";
		else cout << "o_O\n";
	}
	/*int t;
	cin >> t;
	for (int cte = 0; cte < t; ++cte) {
		...
		line_segment a = line_segment({ax, ay}, {bx, by});
		line_segment b = line_segment({cx, cy}, {dx, dy});
		auto res = seg_intersect(a, b);
		if (res.F == 0) {
			cout << "NO\n";
		}
		else if (res.F == 1) {
